...
import cn.edu.wj.rpc.mango.util.ClassUtils;
import cn.edu.wj.rpc.mango.util.ReflectUtils;
import cn.edu.wj.rpc.mango.util.StringUtils;

public class ExtensionLoader<T> {

	private static final String PREFIX = "META-INF/extensions/"; // 扩展服务存放目录

	private static final ConcurrentHashMap<Class<?>, ExtensionLoader<?>> EXTENSION_LOADERS = new ConcurrentHashMap<>();

	private ClassLoader classLoader;

	private Class<T> type; // 加载的class接口类型

	private String defaultExtName;// 默认加载是服务名称
	
    private transient volatile boolean initialized = false; //标识服务是否初始化
    
    private ConcurrentMap<String, Class<T>> extensionClasses;

    private ConcurrentMap<String, T> singletonInstances; //单例实例化存储
	...
	public static <T> ExtensionLoader<T> getExtensionLoader(Class<T> type) {
		...
		if (null == loader) {
			...
			ExtensionLoader<T> oldLoader = (ExtensionLoader<T>) EXTENSION_LOADERS
	...
	// 根据服务接口目录查找需要加载的类
	private synchronized ConcurrentMap<String, Class<T>> loadExtensionClasses(
			String prefix) {
		...
		try {
			...
			throw new IllegalArgumentException(
					"Exception when load extension point(interface: "
							+ type.getName() + ", description file: "
							+ fullName + ").", t);
		}
	}

	//从配置文件加载类
	private void readExtensionClasses0(
			ConcurrentMap<String, Class<T>> extName2Class,
			...
			while ((line = br.readLine()) != null) {
				...
				try {
					...
					if (classLoader == null) {
						...
						clazz = (Class<T>) Class.forName(extImpl, true,
								classLoader);
					}
					// 类型检查
					...
					if (extName2Class.containsKey(name)) {
						if (extName2Class.get(name) != clazz) {
							throw new IllegalStateException(
									"Duplicate extension " + type.getName()
											+ " name " + name + " on "
											+ clazz.getName() + " and "
											+ clazz.getName());
						}
					...
					throw new IllegalStateException(
							"Failed to load config line(" + line
									+ ") of config file(" + url
									+ ") for extension(" + type.getName() + ")",
							e);
				}
			}
		} catch (Throwable t) {
